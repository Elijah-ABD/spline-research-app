...
def catmull_rom_chain(points: tuple, num_points: int, alpha) -> list:
    """
    ...
    all_splines = (catmull_rom_spline(*pq, num_points, alpha) for pq in point_quadruples)
    return flatten(all_splines)


if __name__ == "__main__":
    POINTS: tuple = ((0, 5),(-2, 4), (2, 2), (2.3,2), (5,3), (3,5))  # Red points
    NUM_POINTS: int = 100  # Density of blue chain points between two red points

    #centripetal_points: list = catmull_rom_chain(POINTS, NUM_POINTS, 1)
    #chordal_points: list = catmull_rom_chain(POINTS, NUM_POINTS, 0.5)
    uniform_points: list = catmull_rom_chain(POINTS, NUM_POINTS, 0)


    #plt.plot(*zip(*centripetal_points), c="blue")
    #plt.plot(*zip(*chordal_points), c="green")
    # plt.plot(*zip(*uniform_points), c="black")
